...
def electric_bill(kWh_used):
    ...
    stage3 = 4000
    if (0<kWh_used < 1001):
        consumption = stage1*1000
    elif (1000<kWh_used < 5001):
        consumption = (stage1*1000) + (stage2*(kWh_used-1000))
    elif kWh_used > 5000:
        consumption = (stage1*1000) + (stage2*4000) + (stage3*(kWh_used-5000))
    else:
        consumption = 0
        return consumption
...
def warehouse_process(inventory, transaction):
    if transaction[0] == "buy":
        if transaction[1] in inventory:
            inventory[transaction[1]] += transaction[2]
        else:
            inventory[transaction[1]] = transaction[2]

    elif transaction[0] == "sell":
        if transaction[2] > inventory.get(transaction[1]):
            raise ValueError
        inventory[transaction[1]] -= transaction[2]
...
def fibonacci(n):
    if n == 0:
        ...
        show = fibonacci(n-1) + fibonacci(n-2)
        return show
